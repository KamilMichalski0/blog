...
The Claude Agent SDK for TypeScript enables programmatic building of AI agents with Claude Code's capabilities, allowing them to understand codebases, edit files, run commands, and execute complex workflows.
...
for await (const message of response) {
  if (message.type === 'assistant') {
    ...
    console.log(`Available skills: ${message.skills?.join(', ') || 'none'}`);
  }
}
```
...
// Load project skills by including 'project' in settingSources
const response = query({
...
// Agent can now use loaded skills in its responses
// For example, it might invoke "/label-issue bug priority:high"
// if that skill is available in .claude/commands/
```
          ...
          if (typeof message.content === 'string') {
            ...
            message.content.forEach(block => {
              if (block.type === 'text') {
                ...
                console.log(`Tool request: ${block.name}`);
              }
            });
          }
          break;
...
## Interactive Event-Driven Agent
...
### 1. Use Subagents for Specialization
...
```typescript
canUseTool: async (toolName, input) => {
  // Validate and control tool execution
...
### 4. Use Type-Safe Tools

Leverage Zod for tool schemas:
...
 **Custom Tools** - SDK MCP servers for custom functionality
 **Subagents** - Specialized agents for complex workflows
 **Permission Control** - Granular tool permission management
 **Session Management** - Resume and fork conversations
 **External MCP** - Integrate external MCP servers
 **Settings Loading** - Load project-specific configurations
